...
def NLPContributionGraphEval(submission_dir, gold_dir, output_dir, output_file_prefix, tasks, problem_name):
	
	for task in tasks:
		task_submission_path = os.path.join(submission_dir, task)
		if not os.path.exists(task_submission_path):
			sys.exit('Could not find submission file {0}'.format(task_submission_path))
			
	# evaluate for sentences, and triples
	sentences_total = 0
	sentences_tp = 0
	sentences_fp = 0
	sentences_fn = 0
	...
	for task in tasks:
		...
		for i in range(len(os.listdir(os.path.join(gold_dir, task)))):
			#evaluate sentences	
			gold_reference_path = os.path.join(gold_dir, task, str(i), "sentences.txt")
			task_submission_path = os.path.join(submission_dir, task, str(i), "sentences.txt")
			if not os.path.exists(task_submission_path):
				sys.exit('Could not find submission file {0}'.format(task_submission_path))
			...
			# evaluate phrases
			# gold_reference_path = os.path.join(gold_dir, task, str(i),  "entities.txt")
			# task_submission_path = os.path.join(submission_dir, task, str(i), "entities.txt")
			# if not os.path.exists(task_submission_path):
			# 	sys.exit('Could not find submission file {0}'.format(task_submission_path))
			# total, tp, fp, fn = evaluate(gold_reference_path, task_submission_path)
			...
			#evaluate triples
			gold_reference_path = os.path.join(gold_dir, task, str(i), "triples")
			task_submission_path = os.path.join(submission_dir, task, str(i), "triples")
			if not os.path.exists(task_submission_path):
				sys.exit('Could not find submission file {0}'.format(task_submission_path))
	...
	average_f1 = (sentences_f1 + only_iu_f1 + triples_f1)/3
	
	output_file=open(os.path.join(output_dir, output_file_prefix+'-scores.txt'),"w")
